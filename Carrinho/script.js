...
function totalFunc() {
    ...
    if ($('#delivery').is(':checked')) {
        totalFinal += parseFloat($('#delivery').val());
    }
...
function mensagem() {
    ...
    var mensagemWhats = 'https://api.whatsapp.com/send?l=pt_BR&phone=5565996151325&text=';
    ...
    if ($('#delivery').is(':checked')) {
        let tx = parseFloat($('#delivery').val());
        totalFinal += tx; //parseFloat(tx);
        mensagemWhats += `%0A%0ATaxa de entrega: ${tx.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
    }

    mensagemWhats += `%0A%0ATotal: ${totalFinal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
